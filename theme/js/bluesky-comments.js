...
function renderParentPostStats(statsContainer, parentPostData, statsPreview) {
    ...
    if (finalHtml) {
        ...
        if (statsPreview) {
            statsPreview.innerHTML = previewStatsHtmlParts.filter(part => part).join('');
        }
    } else {
        // Only show if ALL counts were 0
        statsContainer.innerHTML = `<p><em>No interactions yet.</em></p>`;
        if (statsPreview) {
            statsPreview.innerHTML = `<p><em>No interactions yet.</em></p>`;
        }
    }
}
...
async function loadAndRenderComments(targetContainerId, postAtUri) {
    ...
    const statsPreview = document.getElementById('article-title-bsky-stats-preview');
    if (statsPreview) {
        statsPreview.innerHTML = `<p><em>Loading post stats...</em></p>`;
    }
    ...
    try {
        ...
        renderParentPostStats(parentStatsContainer, originalThreadData.post, statsPreview);
